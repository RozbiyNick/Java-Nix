...
public class SignupServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		context.getRequestDispatcher("/jsp/signup.jsp").forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			if(Authenticator.s.signupDataErrors(login, password, password2).size()==0) {
				...
				context.setAttribute("errors", Authenticator.s.signupDataErrors(login, password, password2));
				context.getRequestDispatcher("/jsp/signup.jsp").forward(request, response);
			}
		} catch (Exception e) {
			e.printStackTrace();
